...
Packer.prototype.toJSON = function() {
	res = '{';

	for (var i = 0; i < this.sheets.length; i++) {
		res += '"sheet'+ (i+1) + '":' + this.sheets[i].toJSON();
...
Packer.prototype.toArray = function() {
	var res = [];
	for (var i = 0; i < this.sheets.length; i++) {
		res = res.concat(this.sheets[i].toArray());
	}
...
Packer.prototype.pngExport = function(directory) {
	for (var i = 0; i < this.sheets.length; i++) {
		this.sheets[i].pngExport(directory + '/sheet'+(i+1)+'.png');
	}
};
...
Sheet.prototype.toString = function() {
	...
	for (var i = 0; i < this.lines.length; i++) {
		for (var j = 0; j < this.lines[i].length; j++) {
			...
			res += sprite.name + ': ' + sprite.width + 'x' + sprite.height + ' '+ sprite.x + ' '+ sprite.y + '\n';
		}
	}
	return res+'\n';
};
...
Sheet.prototype.pngExport = function(filename) {
